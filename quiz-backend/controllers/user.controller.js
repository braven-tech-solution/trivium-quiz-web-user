...
exports.updateMe = async (req, res, next) => {
  // console.log(req.user);
  try {
    const result = await updateMeService(req?.user?.email, req.body);
...
exports.getPendingVendors = async (req, res, next) => {
  try {
    const result = await getPendingVendorsService();
    // const { password, ...others } = result;
    res.status(200).json({
      status: "success",
      message: "Pending Vendors List",
      data: result,
    });
  } catch (error) {
    next(error);
...
exports.getApprovedVendors = async (req, res, next) => {
  try {
    const result = await getApprovedVendorsService();
    console.log(result.length);
    // const { password, ...others } = result;
    if (result.length === 0) {
      return res.status(500).json({
        status: "Failed",
        message: "No Vendors existed ",
        data: null,
      });
    }
    res.status(200).json({
...
exports.updateWithGmail = async (req, res, next) => {
  // console.log(req.query.gmailaddress);
  try {
    const result = await updateMeService(req?.query?.gmailaddress, req.body);

    console.log("result", result);

    res.status(200).send({
      status: "success",
      message: "Vendor-ship Approved",
      data: result,
    });
  } catch (err) {
    next(err);
